...
function looksLikeJobSearch(text) {
  return (
    /\b(find|search|looking for|show me|any)\b.*\bjobs?\b/i.test(text) ||
    /\bjobs?\s+(in|near)\b/i.test(text)
  );
}
...
function extractJobIntent(text) {
  ...
  let keyword = text
    .replace(
      /\b(find|search for|looking for|show me|any|jobs?|in\s+[a-z\s]+|near\s+[a-z\s]+|please|for me)\b/gi,
      "",
    )
    .trim();
...
export default function Chatbot() {
  ...
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      content:
        "Hi! I'm the JobXPortal Assistant. I can help you find jobs, improve your resume, prep for interviews, or figure out what to apply for next. What's on your mind?",
    },
  ]);
  ...
  async function sendMessage(text) {
    ...
    const jobIntent = looksLikeJobSearch(userText)
      ? extractJobIntent(userText)
      : null;
...
Respond as the Assistant to the last User message.`;
      ...
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          content: "Sorry, I hit an error just now — mind trying that again?",
        },
      ]);
    } finally {
      setLoading(false);
    ...
    <>
      ...
      >
        {open ? (
          ...
          <MessageCircle size={24} color="#09090B" />
        )}
      </button>
              ...
              >
                ...
                <div
                  style={{
                    ...
                    background: m.role === "user" ? "var(--accent)" : "#FFFFFF",

                    color: m.role === "user" ? "#09090B" : "#374151",
                    ...
                    boxShadow:
                      m.role === "user" ? "none" : "0 2px 8px rgba(0,0,0,0.12)",
                  }}
                >
                  {m.content}
                </div>
                ...
                {m.jobIntent && (
                  ...
                  >
                    🔍 Search "{m.jobIntent.keyword}"
                    {m.jobIntent.location ? ` in ${m.jobIntent.location}` : ""}
                  </button>
                )}
              </div>
            ))}
