...
export function JobsProvider({ children }) {
  ...
  const search = useCallback(async (query) => {
    ...
    const employerJobsPromise = fetchPublicJobsApi({ what: merged.what, where: merged.where })
      .catch(() => ({ jobs: [], count: 0 }));

    // 1) Try JSearch first (primary - real multi-portal data)
    try {
      const [result, employerResult] = await Promise.all([
        fetchJSearchJobs(Object.assign({}, merged, { page: 1 })),
        employerJobsPromise,
      ]);
      providerRef.current = "jsearch";
      const combined = employerResult.jobs.concat(result.jobs);
      setJobs(combined);
      setTotalCount(result.count + employerResult.jobs.length);
    ...
    try {
      const [result, employerResult] = await Promise.all([
        fetchAdzunaJobs(Object.assign({}, merged, { page: 1, resultsPerPage: ADZUNA_RESULTS_PER_PAGE })),
        employerJobsPromise,
      ]);
      ...
      // 3) Fall back to local demo data, still merged with employer-posted jobs
      ...
      const combined = employerResult.jobs.concat(finalFallback);
      setJobs(combined);
